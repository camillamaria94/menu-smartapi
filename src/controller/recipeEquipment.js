const repository = require("../database/repositories/recipeEquipmentRepository")


const create = async (req, res) => {
    const {idRecipe, idEquipment, primary} = req.body;
    if ((!idRecipe) || (!idEquipment)) {
        return res.status(400).json({
            success: false,
            error: "Code is requered."
        })
    }
    const saved = await repository.save({idRecipe, idEquipment, primary })
    console.log(saved) 
...
const edit = async (req, res) => {
    ...
    if (!id) {
        ...
        const edited = await repository.modify({idRecipe, idEquipment, primary })
        console.log(edited)
        return res.status(201).json({
            success: true,
